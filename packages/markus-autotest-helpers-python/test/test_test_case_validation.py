from markus_autotest_helpers_python.test_case_validation import _CaseWrapper, \
...
class TestRunWithReplacements:
    def test_replace_with_correct_internal(self):
        ...
        for test_name in test_cases:
            if 'internal' in test_name:
                assert test_cases[test_name].run(function_to_mock='fixtures.example_tests.internal_buggy',
    ...
    def test_replace_with_correct_external(self):
        ...
        for test_name in test_cases:
            if 'external' in test_name:
                assert test_cases[test_name].run(module_to_replace='fixtures.buggy_function',
                                                 module_to_use='fixtures.correct_function') == ''
    ...
    def test_replace_both(self):
        ...
        for test_name in test_cases:
            assert test_cases[test_name].run(function_to_mock='fixtures.example_tests.internal_buggy',
                                             function_to_use=correct_function,
                                             module_to_replace='fixtures.buggy_function',
                                             module_to_use='fixtures.correct_function') == ''
